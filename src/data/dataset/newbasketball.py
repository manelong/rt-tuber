...
__all__ = ['NewbasketballDetection']
...
class NewBasketball(BaseDataset):
    ...
    def __init__(self, opt, image_filename, pkl_filename):
        ...
        super(NewBasketball, self).__init__(opt, self.ROOT_DATASET_PATH, pkl_filename)

    def imagefile(self, v, i):
        return os.path.join(self.ROOT_DATASET_PATH,'rgb-images', v, '{:0>5}.jpg'.format(i))

@register()
class NewbasketballDetection(NewBasketball, DetDataset):
    __inject__ = ['transforms', ]
    __share__ = ['remap_mscoco_category']
    
    def __init__(self, opt, img_folder, ann_file, transforms=None, remap_mscoco_category=False):
        super(NewbasketballDetection, self).__init__(opt, img_folder, ann_file)
    ...
    #     image, target = super(NewbasketballDetection, self).__getitem__(idx)
